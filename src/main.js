...
const manualJobOverrides = new Map();
...
function relatedPaintingDeliveryIdsForJob(job) {
  const client = state.clients.find((item) => String(item.id) === String(job.client_id));
  const code = String(job.code || "").trim().toLowerCase();
  const title = String(job.title || "").trim().toLowerCase();
  const clientName = String(client?.name || "").trim().toLowerCase();
  return state.paintDeliveries
    .filter((delivery) => {
      const itemJob = expectedPaintingJob(delivery);
      if (itemJob?.id && String(itemJob.id) === String(job.id)) return true;
      const deliveryCode = String(delivery.job_code || "").trim().toLowerCase();
      const deliveryClient = String(delivery.client_name || "").trim().toLowerCase();
      return Boolean((code && deliveryCode === code) || (title && deliveryCode === title) || (clientName && deliveryClient === clientName && (!deliveryCode || deliveryCode === code || deliveryCode === title)));
    })
    .map((delivery) => delivery.id);
}
async function syncPaintingDeliveriesAfterManualJobUpdate(job) {
  if (!job?.has_painting) return;
  const ids = relatedPaintingDeliveryIdsForJob(job);
  if (!ids.length) return;
  const now = new Date().toISOString();
  let patch = null;
  if (job.current_step === "pronto_ritiro") patch = { material_status: "rientrato", checked: true, checked_at: now, updated_at: now };
  else if (["arrivo_officina", "controllo", "in_attesa_cliente"].includes(job.current_step)) patch = { material_status: "ritirato", checked: false, checked_at: null, updated_at: now };
  else if (job.current_step === "verniciatura") patch = { material_status: "consegnato", checked: true, checked_at: now, updated_at: now };
  if (!patch) return;
  const { error } = await supabase.from("painting_deliveries").update(patch).in("id", ids);
  if (error) throw new Error(`Scheda salvata, ma movimenti verniciatura non allineati: ${error.message}`);
  state.paintDeliveries = state.paintDeliveries.map((item) => ids.includes(item.id) ? { ...item, ...patch } : item);
}
...
function fulfillmentLabel(choice) {
  return ({ ritiro: "Ritiro in officina", consegna: "Consegna richiesta", installazione: "Installazione SIMPRO" })[choice] || "Scelta comunicata";
}
function fulfillmentChoice(job) {
  if (job.fulfillment_choice) return `<div class="choice-confirmed ${job.fulfillment_choice === "consegna" ? "delivery-requested" : ""}"><strong>✓ Scelta comunicata: ${esc(fulfillmentLabel(job.fulfillment_choice))}</strong><small>${job.fulfillment_choice === "consegna" ? "SIMPRO ti contatterà per organizzare la consegna a pagamento." : "Puoi modificarla finché SIMPRO non conferma la programmazione."}</small></div>`;
  return `<div class="fulfillment-box"><strong>Come preferisci ricevere il materiale?</strong><small>La consegna è un servizio a pagamento da concordare con SIMPRO.</small><div><button class="secondary fulfillment-choice" data-job="${job.id}" data-choice="ritiro" type="button">Ritiro in officina</button><button class="primary fulfillment-choice" data-job="${job.id}" data-choice="consegna" type="button">Richiedo consegna</button>${job.requires_installation ? `<button class="primary fulfillment-choice" data-job="${job.id}" data-choice="installazione" type="button">Installazione SIMPRO</button>` : ""}</div></div>`;
}
async function submitFulfillmentChoice(token, jobId, choice) {
  const label = fulfillmentLabel(choice);
  ...
  notice(choice === "consegna" ? "Richiesta consegna inviata a SIMPRO. Verrai contattato per organizzarla." : "Scelta inviata a SIMPRO.");
  clientPage(token);
}
...
function subscribeAdminRealtime() {
  ...
  const refresh = (payload) => {
    ...
    const override = payload.table === "jobs" ? manualJobOverrides.get(String(payload.new?.id)) : null;
    if (override && Date.now() < override.expiresAt && payload.new?.current_step !== override.current_step) {
      suppressRealtimeRefreshUntil = Date.now() + 15000;
      supabase.from("jobs").update({
        current_step: override.current_step,
        progress: override.progress,
        phase: override.phase,
        updated_at: new Date().toISOString()
      }).eq("id", payload.new.id).then(({ error }) => { if (error) console.error(error); });
      return;
    }
...
function activityReminder(item) { const job = state.jobs.find((entry) => entry.id === item.job_id); const client = state.clients.find((entry) => entry.id === item.client_id); const isDelivery = String(item.message || "").toLowerCase().includes("consegna"); return `<button class="today-row open-client" data-client="${item.client_id}"><span><strong>${isDelivery ? "🚚" : "🔔"} ${esc(client?.name || "Cliente")}</strong><small>${esc(job?.title || "Lavorazione")} · ${formatDate(item.created_at)}</small></span><b>${isDelivery ? "Consegna" : "Apri"}</b></button>`; }
...
function clientDetail(client, jobs, reminders) { return `<div class="detail-head"><div><small>CLIENTE</small><h2>${esc(client.name)}</h2><p>${esc(client.contact_name || "")}${client.email ? ` · ${esc(client.email)}` : ""}${client.phone ? ` · ${esc(client.phone)}` : ""}</p></div><div class="client-actions"><span class="status ${client.active ? "" : "off"}">${client.active ? "Attivo" : "Disattivato"}</span><button id="edit-client" class="secondary fit">Modifica dati</button><button id="delete-client" class="danger fit">Elimina cliente</button></div></div>${reminders.length ? `<div class="reminders-box"><div class="reminders-title">🔔 RICHIESTE CLIENTE</div>${reminders.map((item) => { const job = jobs.find((entry) => entry.id === item.job_id); const isDelivery = String(item.message || "").toLowerCase().includes("consegna"); return `<div class="reminder-row ${isDelivery ? "delivery-reminder" : ""}"><span><strong>${isDelivery ? "🚚 Consegna da organizzare" : esc(job?.title || "Lavorazione")}</strong><small>${esc(job?.title || "Lavorazione")} · ${formatDate(item.created_at)} · ${esc(item.message || "Richiesta di aggiornamento")}</small></span><button class="handle-reminder" data-id="${item.id}">Segna gestito</button></div>`; }).join("")}</div>` : ""}<div class="link-box"><span>LINK PERSONALE DEL CLIENTE</span><code>${esc(clientLink(client.access_token))}</code><div><button id="copy-link" class="primary fit">Copia link</button><button id="regenerate-link" class="secondary fit">Genera nuovo link</button></div></div><div class="section-title"><div><h3>Lavorazioni</h3><span>${jobs.length} presenti</span></div><button id="new-job" class="primary fit">+ Aggiungi</button></div><div class="work-list">${jobs.length ? jobs.map((job) => { const step = stepFor(job); return `<button class="work-row edit-job ${isOverdue(job) ? "overdue" : ""} ${job.fulfillment_choice === "consegna" ? "delivery-requested-row" : ""}" data-id="${job.id}"><span class="work-icon">${job.fulfillment_choice === "consegna" ? "🚚" : step.icon}</span><span><strong>${esc(job.title)} ${priorityBadge(job)}</strong><small>${esc(job.code || "")} · ${esc(step.label)}${job.painter ? ` · ${esc(job.painter)}` : ""} · Aggiornato il ${formatDate(job.updated_at)}</small>${job.fulfillment_choice ? `<small class="client-choice">Cliente: ${esc(fulfillmentLabel(job.fulfillment_choice))}${job.fulfillment_choice === "consegna" ? " - contattare per costo e data" : ""}</small>` : ""}</span><b>${Number(job.progress)}%</b></button>`; }).join("") : `<div class="empty small"><p>Nessuna lavorazione inserita.</p></div>`}</div>`; }
...
function editJobModal(job) { const photos = state.jobPhotos.filter((photo) => photo.job_id === job.id); const documents = state.jobDocuments.filter((document) => document.job_id === job.id); modal(`<span class="eyebrow red">AGGIORNA LAVORAZIONE</span><h2>${esc(job.title)}</h2>${jobForm(job, true)}${adminPhotoManager(job, photos)}${adminDocumentManager(job, documents)}<button class="complete-job" id="complete-job" type="button">✓ Completa e archivia</button><button class="danger" id="delete-job" type="button">Elimina lavorazione</button>`); bindWorkflowForm(); bindPhotoManager(job); bindDocumentManager(job); document.querySelector("#job-form").onsubmit = async (event) => { event.preventDefault(); const data = Object.fromEntries(new FormData(event.target)); const notifyClient = data.notify_client === "on"; delete data.notify_client; normalizeJobData(data); data.updated_at = new Date().toISOString(); suppressRealtimeRefreshUntil = Date.now() + 15000; manualJobOverrides.set(String(job.id), { current_step: data.current_step, progress: data.progress, phase: data.phase, expiresAt: Date.now() + 45000 }); const { data: updated, error } = await supabase.from("jobs").update(data).eq("id", job.id).select().single(); if (error) { suppressRealtimeRefreshUntil = 0; manualJobOverrides.delete(String(job.id)); return notice(error.message, "error"); } try { if (updated.current_step === "verniciatura" && job.current_step !== "verniciatura") await ensurePaintingDeliveryForJob(updated); await syncPaintingDeliveriesAfterManualJobUpdate(updated); } catch (syncError) { notice(syncError.message, "error"); } await logAction("job", job.id, "update", `Aggiornata lavorazione ${data.title}: ${data.phase}`); state.jobs = state.jobs.map((item) => String(item.id) === String(job.id) ? { ...item, ...updated } : item); closeModal(); renderAdmin(); if (notifyClient) { const { error: emailError } = await supabase.functions.invoke("notify-client", { body: { job_id: job.id } }); notice(emailError ? "Aggiornamento salvato, ma email non inviata." : "Aggiornamento salvato ed email inviata.", emailError ? "error" : "ok"); } else notice("Aggiornamento salvato."); }; document.querySelector("#complete-job").onclick = () => completeJobModal(job); document.querySelector("#delete-job").onclick = async () => { if (!confirm(`Eliminare definitivamente la lavorazione "${job.title}"?`)) return; const { error } = await supabase.from("jobs").delete().eq("id", job.id); if (error) return notice(error.message, "error"); await logAction("job", job.id, "delete", `Eliminata lavorazione ${job.title}`); notice("Lavorazione eliminata."); adminPage(); }; }
...
async function driverPage() {
  ...
  document.querySelector("#driver-form").onsubmit = async (event) => {
    ...
    try {
      ...
      form.reset(); form.driver_name.value = "Gabriele"; selectedNode.hidden = true; searchInput.value = ""; renderSearchResults(""); message.textContent = "Movimento registrato correttamente."; message.className = "form-message ok"; await loadDriverDeliveries();
    } catch (error) {
